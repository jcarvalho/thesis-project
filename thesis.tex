...
\begin{abstract}
  ...
  extension to PSTM's to support the concept of Long-Lived
...
understand this concept, consider the following example, found in the
applications of many education institutions.
...
\begin{figure}
  ...
  \begin{tikzpicture}
    ...
    node[below]{Read Set} (Log)
    node[near start, left]{1}
    node[near end, below]{*};

    \draw [umlcd style school] (TransactionalContext) --    
    node[right]{Write Set} (Log)
    node[near start, above]{1}
    node[near end, right]{*};

    \draw [umlcd style school] (DomainObject) --
    node[left]{Object} (Log)
    node[near start, above]{1}
    node[near end, left]{*};

    \draw [umlcd style school] (DomainObject) |-
    node[below]{Value} (Log)
    node[near start, right]{1}
    node[near end, below]{1};
