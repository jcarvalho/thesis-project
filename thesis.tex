...
transparently at an infrastructural level (meaning that the programmer should
...
top of DBMS's. Those applications typically have requirements similar
to those with a rich domain model, however, they must rely directly on
...
Transactions are a age-old notion in the database world. They comprise
a ``Unit of Work'' performed within a Database Management System, and
ensure that concurrent access to shared mutable data is done in a
consistent way.

By definition, a database transaction must provide the four ACID properties:

\begin{itemize}

\item {\bf Atomicity} Requires that in a transaction, all database
  operations either {\it all} occur, or {\it nothing} occurs. This
...
first law of Concurrency Control, by the Database-guru Jim Gray. In
fact, concurrency control mechanisms are what ensure that the ACID
properties of transactions are kept under an environment with
concurrent access to shared mutable data. Research on the topic dates
back to the 1970s\cite{rosenkrantz1978system} and
1980s\cite{gray1981transaction}, and is still a hot topic nowadays.
...
\item {\bf Optimistic} Delaying integrity checking until the end of
  ...
  Optimistic concurrency control allows for greater concurrency, since
  concurrent operations can proceed without interfering with each
  other. On the other hand, given that integrity is only checked in
  the end of the transaction, conflicts are not detected until all the
  work is done and the transaction has to be restarted. It is
...
category, and most Relational Databases implement it.
...
only ``active'' transactions (the ones currently updating the
database) hold locks. ``Sleeping'' transactions (not currently
updating the database - also known as ``User think-time'') do not hold
any locks. This means that the isolation property is broken, since
other transactions will be able to see an uncommitted value.

In \cite{salem1989altruistic}, the authors propose an extension to
Two-Phase locking, called altruistic locking. This extension provides
the concurrency control manager with a set of rules that allow it to
release locks early in the transaction. The altruistic protocol takes
advantage of the transaction's data access pattern. In particular, it
takes advantage of the knowledge that a transaction no longer needs
access to a data object that is has locked. Transactions can then
issue ``release'' operations, indicating that a certain piece of data
will no longer be accessed. Other transactions will be allowed to
concurrently access this data, while agreeing to abide by certain
restrictions, put in place to ensure that all transactions see a
...
Long-Lived Transactions must be aborted since some of their log
...
Long-Lived Transactions, so they will not be aborted for that
...
(in fact, each step is typically executed by a different users), and
...
Since their support for recovery and failure handling is lacking,
...
Section~\ref{sec:sagas}. In this section, I will show how sagas can be
...
literature\cite{798492} \cite{alonso1996advanced}. However, such
...
interfaces. As we have seen, lock-based approaches do not cope well
with this principle, since the programmer must be aware of module's
internal locking conventions. Transactions however do not have this
...
must be done if the transactions needs to be rolled-back is
increased. Using nested transactions, smaller portions of the work can
be done in isolation, each in a nested transaction. If one of those
portions fails, only that portion needs to be rolled-back, and an
attempt can be made to compensate for that failure. Similarly, clients
of opaque libraries may suffer performance hits if the transaction
fails because of the work performed in said libraries.
...
is achieved, since resources may be released earlier and conflict
...
basic principle as the tecniques described in previous sections, based
on the relaxation of transactional properies, and compensating
...
\clearpage \bibliography{thesis.bib} \bibliographystyle{plain}
\end{document}
