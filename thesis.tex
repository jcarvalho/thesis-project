...
in applications with Rich Domain Models.  Rich Domain Models are ``An
...
deadlocks, starve the database's connection pool. All these factors
...
new value to. It is important to note that there is at most one Log
entry per object. When a write to an object that has been previously
read occurs, its corresponding Log entry is promoted from the Read Set
to the Write Set.
...
I previously presented concept of Transactional Context as the basic
building blocks for Long-Lived Transactions. However, useful as they
may be,Transactional Contexts are not business operations.
...
\begin{interface}[text width=2.5cm]{@Step}{3, -2}
  \attribute{last : boolean}
\end{interface}
...
with the @Step annotation. This will assure that any invocations the
method will execute within the context of that
...
current transactional context, which might be stored in a thread-local
variable.
...
\lstset{language=Java, frame=single, caption={Code to ensure each step
  ...
  label={lst:tcMethod}}
\begin{lstlisting}
TransactionalContext context = getCurrentTC();
try {
...
be directly associated to its logical 'owner' (e.g. the user who
created it, the department it belongs to, etc).

Getting a reference to the TBO between its various steps (remember the
application might be restarted, losing all session data) might be as
...
\item By invoking the end() method of the TBO, which will attempt to
  ...
  is successful, both the TBO and its context as deleted.
...
\item By invoking a method annotated with @Step, with the {\it last}
  ...
  once its last step is invoked. The semantics of 'committing' in this
  case are similar to the ones provided by calling end(). This is the
  preferable way of ending a TBO, since, in my opinion, it is the most
...
operation until it commit) and the department for which the course is
being created.

An implementation of the CourseCreationOperation class is presented in
listing~\ref{lst:courseTBO}. Each step of the user interaction is
implemented in a single method, annotated with @Step. Since defining
the bibliography is the last step of the operation, the
'setBibliography()' is annotated with 'last = true', meaning that
after its invocation, the operation will attempt to commit, making the
new course visible to every other user in the system.
