...
application, and in 2008 was extracted from the code base, into a
standalone Java framework to develop applications that require a
transactional and persistent domain model.

In the Fenix Framework, the programmer describes the domain model of
the application in the DML, and the Framework is responsible for
...
values of the application's shared mutable state. Unlike other STMs,
read-only transactions never conflict with any other concurrent
transaction.
...
details on the Framework's implementation, see \cite{fernandes2011strict}.
...
Manager and Persistence Backend. In this new architecture, the
Framework can work on top of any transaction manager, be it an STM or
not.

Support for Long-Lived Transactions will be build using the JVSTM as
its transaction manager, but will be agnostic to the persistence
backend.
...
\item {\bf Detection} Awareness that the current transaction's state
  is inconsistent and cannot be committed. In short transactions,
  it is usually simple to restart the transaction once a conflict is
  ...
  transaction from committing, pinpointing the cause of the
  problem.
