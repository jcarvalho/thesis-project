...
\item {\bf Serializable} is the {\it highest} isolation level defined
  ...
  collection. This can occur when running queries
  like ``SELECT * FROM users WHERE age BETWEEN 10 AND 30;''. If an
  interleaving transaction creates a new user with an age between
  10 and 30 years, the second run of the query will show a result
  which contains the new user.
\end{itemize}
...
\subsubsection{Supporting Long-Lived Transactions in the DBMS}
