...
programmer should be mostly unaware of the persistence layer).
...
for transactional support, and thus, it becomes clear that DBMSs
should support Long-Lived Transactions.
...
time, it fills up, thus forcing Long-Lived Transactions to abort,
because some of their log records will be overwritten by more recent
transactions.
...
To provide transactional support for domain objects, ORM's typically
...
the fact that the operation may have several steps, we must keep
