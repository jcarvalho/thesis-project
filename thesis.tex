...
\begin{abstract}
  ...
  Transactional Memories (PSTM) was born. In this project, I propose an
  extension to PSTMs to support the concept of Long-Lived
  Transactions. My thesis is that supporting Long-Lived Transactions
  should be done at the infrastructural level on Persistent STMs, so
  I will describe how those systems can be extended to
...
With the adoption of multicore architectures, {\it Software
  Transactional Memory} (STM) has seen many advancements over the past
years, and enterprise applications are now beginning to use them for
transactional support \cite{fernandes2011strict}, instead of relying
on the Database.

Long-Lived Transactions were first described in 1981 as ``[..]
transactions with lifetimes of a few days or
weeks''\cite{gray1981transaction}. Many enterprise applications have
requirements for such transactions, and thus, several approaches were
devised to cope with them, as described in Section~\ref{sec:related}.
...
evaluated. Finally, in Section 6, I shall draw some conclusions.
...
all solutions require the programmer to 
...
\begin{figure}
  ...
  \begin{tikzpicture}
    ...
    node[below]{Read Set} (Log) node[near start, left]{1} node[near
    end, below]{*};

    \draw [umlcd style school] (TransactionalContext) --
    node[right]{Write Set} (Log) node[near start, above]{1} node[near
    end, right]{*};

    \draw [umlcd style school] (DomainObject) -- node[left]{Object}
    (Log) node[near start, above]{1} node[near end, left]{*};

    \draw [umlcd style school] (DomainObject) |- node[below]{Value}
    (Log) node[near start, right]{1} node[near end, below]{1};
